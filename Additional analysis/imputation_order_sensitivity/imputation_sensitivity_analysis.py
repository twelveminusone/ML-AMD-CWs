...
# -*- coding: utf-8 -*-
"""
Imputation-Order Sensitivity — Global (Leakage Baseline)
--------------------------------------------------------
Evaluate a *global* imputation strategy where the imputer is fit on the
entire dataset (train+test together), then simulate-missing on KFold
splits to quantify potential optimistic bias due to leakage.
...
Example:
  python imputation_sensitivity_analysis.py \
    --input ../../data/initial_dataset.xlsx \
    --output ../../outputs/imputation_order_sensitivity/global/results.xlsx \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Global (leakage) imputation baseline with KFold simulate-missing."
    )
    p.add_argument(
        "--input",
        type=Path,
    ...
    p.add_argument(
        "--output",
        type=Path,
        default=Path("../../outputs/imputation_order_sensitivity/global/results.xlsx"),
...
# ------------------------
# Core evaluation (GLOBAL fit)
# ------------------------
def eval_feature_global_kfold(
    df: pd.DataFrame,
    target_col: str,
    ...
    For one target column that has missing values:
      - Consider rows where target is non-missing (to have ground truth).
      - For each fold, set target in TEST rows to NaN (simulate-missing),
        FIT the imputer on the *entire* array (train+test together),
        then transform and compare imputed values on TEST vs ground truth.
    """
    mask = df[target_col].notna()
    X_full = df.loc[mask].drop(columns=[target_col]).to_numpy()
    ...
    # Methods: estimators dict + MICE (estimator=None)
    full_methods = {**estimators, "MICE": None}

    # Build full array once (X + y)
    arr_all_base = np.hstack([X_full, y_full.reshape(-1, 1)])

    for method_name, base_est in full_methods.items():
        r2_list, rmse_list, mae_list = [], [], []

        for fold_id, (_, te_idx) in enumerate(kf.split(X_full), start=1):
            # Copy and simulate-missing on TEST rows only
            arr_all = arr_all_base.copy()
            y_true = y_full[te_idx]
            arr_all[te_idx, -1] = np.nan

            try:
                # GLOBAL fit: fit the imputer on the entire array (with test target masked)
                imputer = make_iterative_imputer(base_est, seed=seed)
                imputer.fit(arr_all)
                arr_imp = imputer.transform(arr_all)
                y_pred = arr_imp[te_idx, -1]
                ...
                fold_rows.append(
                    {
                        ...
                        "note": "Global-fit baseline (leakage).",
                    }
                )
...
def run(args: argparse.Namespace) -> None:
    ...
    # KFold for simulate-missing aggregation (even though imputer fits globally)
    ...
    print(f"[INFO] This is a GLOBAL-FIT baseline (imputer sees all rows).")

    for col in missing_cols:
        summary_rows, fold_rows = eval_feature_global_kfold(
            df=df,
            target_col=col,
